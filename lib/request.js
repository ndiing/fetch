...
class Request {
     ...
     *
     * @param {string} input - URL untuk permintaan. Dapat mengandung parameter dinamis dalam format `:paramName`.
     * @param {Object} [options={}] - Opsi untuk permintaan.
     * @param {Object} [options.params={}] - Parameter untuk menggantikan parameter dinamis dalam URL.
     * @param {Object} [options.query={}] - Parameter kueri untuk menambahkan ke URL.
     * @param {string|Readable} [options.body=""] - Isi dari permintaan. Jika berupa string, akan diubah menjadi stream.
     * @param {string} [options.credentials="same-origin"] - Kredensial untuk permintaan (default: "same-origin").
     * @param {Object} [options.headers={}] - Header untuk permintaan. Nilai default termasuk header umum untuk browser.
     ...
     * @param {Object} [options.agent] - Agen untuk permintaan. Jika tidak disediakan, agen dengan `keepAlive` diaktifkan dan `rejectUnauthorized` dinonaktifkan.
    ...
    constructor(input, options = {}) {
        if (options.params) {
            input = input.replace(/:(\w+)/g, ($, name) => options.params[name] ?? "");
        }
        ...
        if (options.query) {
            for (const name in options.query) {
                ...
                this.input.searchParams.set(name, value);
            }
        }
